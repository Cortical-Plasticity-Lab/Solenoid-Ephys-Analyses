function T = addICMSLFPbetas(T,C,K)
%ADDICMSLFPBETAS Add Betas for solenoid-LFP RMS principal components
%
%  T = tbl.stats.addICMSLFPbetas(T);
%  T = tbl.stats.addICMSLFPbetas(T,C,K);
%
% Inputs
%  T - Main data table
...
for iT = 1:size(TID,1)
   if TID.Type(iT)~="ICMS"
      TID.coeff{iT} = TID.coeff{TID.Type=="ICMS" & TID.BlockID==TID.BlockID(iT)};
   end
end
