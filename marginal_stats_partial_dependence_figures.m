function fig = marginal_stats_partial_dependence_figures(mdl)
%MARGINAL_STATS_PARTIAL_DEPENDENCE_FIGURES  For comparing glme with polyfit
%
% Syntax:
%   fig = marginal_stats_partial_dependence_figures(mdl);
%
% Inputs:
%   mdl - Struct with statistical models in it, generated by
%           `run_stats_pca` and saved in `run_stats_pca_output.mat`.
%
% Output:
%   fig - (Optional) figure handle array. If this is not specified, then
%           the figures are generated, displayed for 5 seconds, then saved
%           in `figures/marginal_stats_comparison` and closed.
%
